...
exports.test = function(options, callback) {
    ...
    let new_url = `mongodb://${options.user}:${options.pass}@${options.host}:${options.port}/${options.db}?authSource=admin`
...
exports.connect = function(callback) {
    async.waterfall([
        ...
        function(config, callback) {
            // let raw_url = 'mongodb://' + config.host + ':' + config.port + '/' + config.db
            // mongoose.open
            var db = mongoose.createConnection();
            let new_url = `mongodb://${config.user}:${config.pass}@${config.host}:${config.port}/${config.db}?authSource=admin`
            // console.log(`###new_url####`, new_url);
            // db.openUri(new_url, {
